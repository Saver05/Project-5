...
void *barber(void *arg) {
    ...
    while (1) {
        ...
        Zem_post(&cut_done[barber_id]);
        Zem_post(&barber_free[barber_id]);
...
void *customer(void *arg) {
    ...
    for (int i = 0; i < NUM_BARBERS; i++) {
        if (barber_free[i].value > 0) {
            Zem_wait(&barber_free[i]);
            info->barber_id = i;
            assigned_barber = i;
    ...
    Zem_post(&mutex);
    Zem_wait(&cut_done[assigned_barber]);
...
int main() {
    ...
    barber_free = malloc(NUM_BARBERS * sizeof(Zem_t));
    cut_done = malloc(NUM_BARBERS * sizeof(Zem_t));
    for (int i = 0; i < NUM_BARBERS; i++) {
        Zem_init(&customer_ready[i], 0);
        Zem_init(&barber_free[i], 1);
        Zem_init(&cut_done[i], 0);
